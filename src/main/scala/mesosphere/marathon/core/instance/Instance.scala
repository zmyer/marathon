...
object Instance {
  ...
  def apply(): Instance = {
    new Instance(Instance.Id(""), AgentInfo("", None, Nil),
      InstanceState(Condition.Unknown, Timestamp.zero, Timestamp.zero, healthy = None), Map.empty[Task.Id, Task])
  }
  ...
  case class InstanceState(condition: Condition, since: Timestamp, version: Timestamp, healthy: Option[Boolean])
  ...
  private[instance] def newInstanceState(
    ...
    val stateMap = tasks.groupBy(_.status.taskCondition)
    val status = if (stateMap.size == 1) {
      ...
      val distinctStates = Instance.AllInstanceConditions.foldLeft(stateMap) { (ds, status) => ds - status }
      Instance.DistinctInstanceConditions.find(distinctStates.contains).getOrElse {
        // if no distinct state is found all tasks are in different AllInstanceConditions
        // we pick the first matching one
        Instance.AllInstanceConditions.find(stateMap.contains).getOrElse {
    ...
    maybeOldState match {
      case Some(state) if state.condition == status && state.healthy == healthy => state
  ...
  private[this] def isPending(task: Task): Boolean = {
    task.status.taskCondition != Condition.Running && task.status.taskCondition != Condition.Finished
  }
