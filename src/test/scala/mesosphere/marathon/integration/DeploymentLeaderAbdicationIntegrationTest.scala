...
abstract class DeploymentLeaderAbdicationIntegrationTest extends LeaderIntegrationTest with StrictLogging {
  ...
  def test(appId: PathId, createApp: AppDefinition, updateApp: AppUpdate) = {
    ...
    logger.info(s"Started app: ${marathon.app(appId).entityPrettyJsonString}")
    ...
    logger.info(s"Updated app: ${marathon.app(appId).entityPrettyJsonString}")
    ...
    logger.info(s"App after restart: ${firstRunningProcess.client.app(appId).entityPrettyJsonString}")
