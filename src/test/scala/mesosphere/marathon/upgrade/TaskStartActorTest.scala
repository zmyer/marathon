...
class TaskStartActorTest
  ...
  ignore("Start success with dying existing task, reschedules, but finishes early") {
    ...
    system.eventStream.publish(f.instanceChange(app, instanceId, Condition.Error).copy(runSpecVersion = outdatedInstance.tasks.head.launched.get.runSpecVersion))
