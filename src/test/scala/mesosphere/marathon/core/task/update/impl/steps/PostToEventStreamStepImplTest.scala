...
class PostToEventStreamStepImplTest extends FunSuite
  ...
  class Fixture(system: ActorSystem) {
    ...
    private[this] val healthyInstanceState = InstanceState(Condition.Running, Timestamp.now(), Timestamp.now(), Some(true))
    private[this] val unhealthyInstanceState = InstanceState(Condition.Running, Timestamp.now(), Timestamp.now(), Some(false))
