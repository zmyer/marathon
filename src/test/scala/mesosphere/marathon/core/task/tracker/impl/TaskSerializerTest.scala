...
class TaskSerializerTest extends FunSuite with Mockito with Matchers with GivenWhenThen {
  ...
  class Fixture {
    ...
    object Resident {
      ...
      private[this] val status = Task.Status(stagedAt, Some(startedAt), Some(mesosStatus), taskCondition = Condition.Running)
      ...
      def reservedState = Task.Reserved(
        ...
        status = Task.Status(stagedAt = Timestamp(0), taskCondition = Condition.Reserved)
      )
