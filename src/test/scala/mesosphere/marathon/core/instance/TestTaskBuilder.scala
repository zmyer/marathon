...
object TestTaskBuilder {
  ...
  object Helper {
    ...
    def minimalUnreachableTask(appId: PathId, taskCondition: Condition = Condition.Unreachable, since: Timestamp = Timestamp.now()): Task.LaunchedEphemeral = {
      ...
      lostTask.copy(status = lostTask.status.copy(taskCondition = taskCondition))
    }
    ...
    def minimalReservedTask(appId: PathId, reservation: Task.Reservation, instance: Option[Instance] = None): Task.Reserved =
      Task.Reserved(
        ...
        status = Task.Status(Timestamp.now(), taskCondition = Condition.Reserved))
    ...
    def taskLaunched: Task.Launched = {
      ...
      Task.Launched(now, status = Task.Status(stagedAt = now, taskCondition = Condition.Running), hostPorts = Seq.empty)
    }
    ...
    def startingTask(taskId: Task.Id, appVersion: Timestamp = Timestamp(1), stagedAt: Long = 2): Task.LaunchedEphemeral =
      Task.LaunchedEphemeral(
        ...
        status = Task.Status(
          ...
          taskCondition = Condition.Starting
        ),
        hostPorts = Seq.empty
      )
    ...
    def stagedTask(
      ...
      Task.LaunchedEphemeral(
        ...
        status = Task.Status(
          ...
          taskCondition = Condition.Staging
        ),
        hostPorts = Seq.empty
      )
