...
case class TestInstanceBuilder(
  ...
  def taskLaunchedOp(): InstanceUpdateOperation.LaunchOnReservation = {
    InstanceUpdateOperation.LaunchOnReservation(instanceId = instance.instanceId, timestamp = now, runSpecVersion = instance.runSpecVersion, status = Task.Status(stagedAt = now, taskCondition = Condition.Running), hostPorts = Seq.empty)
  }
...
object TestInstanceBuilder {
  ...
  def emptyInstance(now: Timestamp = Timestamp.now(), version: Timestamp = Timestamp.zero, instanceId: Instance.Id): Instance = Instance(
    ...
    state = InstanceState(Condition.Created, now, version, healthy = None),
    tasksMap = Map.empty
  )
